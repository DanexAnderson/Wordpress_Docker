...
function universityRegisterSearch() { // Vreate of custom API route
...
function universitySearchResults($data) // URL =>  http://localhost:3000/wp-json/university/v1/search
{
    ...
    while($mainQuery->have_posts()) {
        ...
        if (get_post_type() == 'professor') {
            ...
            'title' => get_the_title(),
            'permalink' => get_the_permalink(),
            'image' => get_the_post_thumbnail_url(0, 'professorLandscape' ) // 0 for current post
        ) );
        }
    ...
    $programsMetaQuery = array('relation' => 'OR');// To return multiple arrays results of a search key
    ...
    $programRelationshipQuery = new WP_Query(array(  // wordpress sql Query . array contains the conditions
